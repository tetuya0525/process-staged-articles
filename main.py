...
class Config:
    ...
    def validate(self):
        ...
        if not self.GCP_PROJECT_ID or not self.INTEGRATE_ARTICLE_TOPIC_ID:
            raise ValueError("GCP_PROJECT_ID and INTEGRATE_ARTICLE_TOPIC_ID are required.")
...
class Validator:
    ...
    def validate_message(data: Dict[str, Any]) -> Tuple[str, str]:
        ...
        if not doc_id: raise ValidationError("メッセージにdocumentIdが含まれていません")
        return doc_id, batch_id
    @staticmethod
    def validate_document(doc_data: Optional[Dict[str, Any]], doc_id: str) -> None:
        if not doc_data: raise ValidationError(f"ドキュメントが存在しません: {doc_id}")
        if doc_data.get('status') != DocumentStatus.QUEUED.value: raise ValidationError(f"ドキュメントは処理対象外です (status: {doc_data.get('status')})")
        if not doc_data.get('content', {}).get('rawText'): raise ValidationError(f"ドキュメントに処理対象のテキストがありません: {doc_id}")
...
class ArticleProcessor:
    # ... (v2.1と同じロジック)
    def __init__(self, db: FirestoreClientProtocol, publisher: PubSubPublisherProtocol, config: Config):
        self.db = db
    ...
    def _simulate_ai_processing(self, raw_text: str) -> Dict[str, Any]:
        ...
        return {'summary': f"本文の文字数は{text_length}文字です。", 'categories': categories, 'tags': tags}
    ...
    def process(self, doc_id: str, batch_id: str) -> bool:
        ...
        def _process_in_transaction(trans):
            ...
            final_update = {
                'status': DocumentStatus.PROCESSED.value, 'aiGenerated': ai_results,
                'updatedAt': firestore.SERVER_TIMESTAMP, 'batchId': batch_id
            }
            trans.update(doc_ref, final_update)
            return True
        result = _process_in_transaction(transaction)
        if result:
            topic_path = self.publisher.topic_path(self.config.GCP_PROJECT_ID, self.config.INTEGRATE_ARTICLE_TOPIC_ID)
            next_message = json.dumps({"documentId": doc_id, "batchId": batch_id}).encode('utf-8')
            future = self.publisher.publish(topic_path, next_message)
            future.result(timeout=30)
            log_structured('INFO', "次のワークフローへメッセージを送信", document_id=doc_id)
...
def create_app() -> Flask:
    ...
    def health_check():
        try:
            ...
            current_app.pubsub_publisher.topic_path(current_app.config['APP_CONFIG'].GCP_PROJECT_ID, "health-check")
